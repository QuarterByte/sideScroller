...
sk = pygame.image.load(os.path.join("Assets/Other", "Sky.png"))
...
class Dinosaur:
    ...
    def duck(self):
        self.image = self.duck_img[self.step_index // 24]  
        self.dino_rect = self.image.get_rect() 
        self.dino_rect.x = self.X_POS
...
def main():
    global game_speed, x_pos_bg, y_pos_bg, points, obstacles, d
    run = True
...
def menu(death_count):
    ...
    while run:
        ...
        SCREEN.blit(text, textRect)
        SCREEN.blit(RUNNING[3], (SCREEN_WIDTH // 2 - 30, SCREEN_HEIGHT // 2 - 150))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
